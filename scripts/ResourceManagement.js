...
function ReleaseSpecific(){
	var cgiUrl = 'https://'+CURRENT_IP+'/cgi-bin/Final/RM/RM.py?action=cancel&query=ResourceId='+globalResourceId;
	$.ajax({
...
function checkExtensionLimit(user,resid) {

	var ret = "";
	$.ajax({
	
		url:'../php/functions_receiver.php?action=checkextensionlimit&user='+user+'&resourceid='+resid,
		dataType: 'text/xml',
		async: false,
		success:function(data) {
			ret = $.trim(data);
...
function checkUnprovisioningStatus(user, id) {

	var retcerd = "";

	var url = '../php/functions_receiver.php?action=checkunprovisioningstatus&user='+globalUser+'&resourceid='+id;
	$.ajax({
		url: url,
		dataType: 'text/xml',
		async: false,
...
function getActiveReservationIteration(resid) {

	var ret = "";
	// alerts(q);
	$.ajax({
	
		url:'../php/functions_receiver.php?action=getiterleft&resid='+resid,
		dataType: 'text/xml',
		async: false,
		success:function(data) {
			ret = $.trim(data);
...
function getDevicesFromSelectedReservation() {
	...
	$('#'+refreshFlagResource+'Table input:checkbox[name="'+refreshFlagResource+'MatrixSel"]').each(function() {
   		if ($(this).is(':checked')){
			var ridArr = $(this).val().split("*");
			ids.push(ridArr[0]);
		}
	});
	$('#'+refreshFlagResource+'Table input:checkbox[name="'+refreshFlagResource+'ConfigSel"]').each(function() {
   		if ($(this).is(':checked')){
			var ridArr = $(this).val();
			ids.push(ridArr);
		}
	...
	var url = "../php/functions_receiver.php?action=getseldev2&selected="+ids;
	...
	$.ajax({
		...
		success: function(data) {
            ...
            for (var x = 0; x < dataArr.length; x++) {
                ...
                str += "<td style='border: 1px solid #FFFBFF'><input type='checkbox' name='specDevSel' value='"+dataInfo[0]+"' onclick='enableDisableSpecific(this.value,this)' />&nbsp;"+dataInfo[1]+"</td>";
                cnt++;
            }
            str += "</tr></table></div>";
